# 🛍️ Online Retail Data Analysis & Power BI Dashboard

## 📊 Project Overview
This project focuses on analyzing an online retail dataset using Excel, Power Query, and Power BI.  
The objective is to clean, transform, and analyze transactional data to generate meaningful business insights and build an interactive dashboard.

---

## 🧹 Data Cleaning (Excel)
Initial data preparation was performed in Excel:
- Removed duplicate records  
- Handled missing and inconsistent values  
- Corrected data types (dates, numeric fields)  
- Structured raw data for further processing  

---

## 🔄 Data Transformation (Power Query)
Power Query was used to:
- Clean and filter raw transactional data  
- Transform and reshape datasets  
- Create calculated columns  
- Prepare data for analytical use in Power BI  

---

## 📈 Data Analysis
The following analyses were performed on the dataset:

- Customer segmentation using RFM analysis (Recency, Frequency, Monetary)  
- Customer distribution across RFM segments  
- Monthly sales trend analysis  
- Order distribution and identification of peak hours  
- Sales contribution by customer segments  
- Top 5 countries based on total sales and quantity sold  
- Top 10 customers by revenue contribution  

---

## 📊 Dashboard (Power BI)
An interactive dashboard was developed in Power BI featuring:
- Dynamic visualizations and charts  
- Key business metrics and KPIs  
- Filters and slicers for interactive analysis  
- Clear layout for data exploration and decision support  

---

## 🛠 Tools Used
- Microsoft Excel  
- Power Query  
- Power BI  

---

## 📷 Dashboard Preview

![Dashboard Preview](https://raw.githubusercontent.com/ParnazAli/online-retail-data-analysis/refs/heads/main/Final_Dashboard.jpg)

---

## 🎯 Purpose
This project was created to develop and demonstrate practical skills in data cleaning, transformation, and data visualization using modern analytics tools.
